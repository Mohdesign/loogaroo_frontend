...
export default function EditServices({ service, allCasestudies }) {
    ...
    // get the page name
    const pageName = service.data.attributes.title;
 
    if(service) {
        console.log(service.data.attributes.associate_casestudies.data);
    }
    ...
    // Input Fields
    const[published_At, setPublished_At] = useState(service.data.attributes.publishedAt);    
    
    const [ values, setValues ] = useState({
        title: service.data.attributes.title,
        content: service.data.attributes.content,
        subtitle: service.data.attributes.subtitle,
        slug: service.data.attributes.slug,
        associate_casestudies: service.data.attributes.associate_casestudies.data,
    ...
    let formData = new FormData();
        ...
        axios({
            method: "put",
            url: `${API_URL}/api/services/${service.data.id}`,
            data: formData,
        })
        .then(({ data }) => {
          setResponse(data);
    ...
    return (
        ...
        <Box background="neutral100" padding={0}>
           ...
           primaryAction={
            ...
            >  
            <a> Go back</a>
            </Link>} title={pageName} subtitle={"Edit service entry"} as="h2" />
        </Box>
                            ...
                            allCasestudies.data.map(casestudy => (
                                <div className="custom-checkbox"  key={casestudy.id}>
                                     <BaseCheckbox  aria-label="Simple checkbox" 
                                        type="checkbox"
                                        checked={associate_casestudies.find(single => single.id === casestudy.id)}
                                        onChange={(e) => {
                                            if(e.target.checked) {
                                                setValues({
                                                    ...values,
                                                    associate_casestudies: [...values.associate_casestudies, casestudy]
                                                })
                                            } else {
                                                setValues({
                                                    ...values,
                                                    associate_casestudies: values.associate_casestudies.filter(single => single.id !== casestudy.id)
                                                })
                                            }
...
export async function getServerSideProps(context) {
    ...
    const res = await axios.get(`${API_URL}/api/services/${id}?populate=*`);
    const res2 = await axios.get(`${API_URL}/api/casestudies`);
